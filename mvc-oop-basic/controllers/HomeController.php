    ...
    public function addGioHang(){
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            if (isset($_SESSION['user_client'])) {
                ...
                header("Location:" . BASE_URL . '?act=gio-hang&tab=orders');
            }else{
...
{
    // nếu chưa có view thì dùng tạm
    if (file_exists('views/download.php')) {
        require_once 'views/download.php';
    } else {
        echo "<h3>Chưa có trang Download</h3>";
    }
}
// ================== PAYMENT ==================
public function payment()
{
    if (file_exists('views/payment.php')) {
        require_once 'views/payment.php';
    } else {
        echo "<h3>Chưa có trang Thanh toán</h3>";
    }
}
